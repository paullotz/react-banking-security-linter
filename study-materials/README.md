...
The application contains 3 hidden security vulnerabilities across 2 pages. Participants in Phase 1 search manually without the plugin. In Phase 2 they use the plugin to find them immediately.
...
| Stored | `localStorage.setItem('accessToken', payload.accessToken)` and `localStorage.setItem('sessionRef', payload.sessionId)` |
